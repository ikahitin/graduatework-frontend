...
import ExchangeApartmentItems from "../../components/ExchangeApartmentItems";

export default function ExchangeVacationResult({itemsPerPage}) {
    ...
    const [proposedCity, setProposedCity] = useState(searchParams.get("proposed_city"));
    ...
    React.useEffect(() => {
        const desiredCity = `city=${destination}`
        const proposedCity = `proposed_city=${destination}`
        const dates = `start=${startDate}&end=${moment(endDate).format('YYYY-MM-DD')}`
        const details = `adults=${adults}&children=${children}`
        const url = `exchange_apartment?${desiredCity}&${proposedCity}&${dates}&${details}`;
    ...
    return (
        <div className="container col-10 search">
            ...
            <div className="breadcrumb">
                ...
                <NavLink end={true} to="/booking/vacation">Відпочинок за обміном</NavLink>
                    ...
                    <div className="filter">
                        ...
                        <div className="search-options">
                            <div className="input-block">
                                <span className="input-tip">Місце, куди хочете вирушити </span>
                            ...
                            <div className="input-block">
                                <span className="input-tip">Місце, яке ви пропонуєте</span>
                                <input type="text" className="form-control simple house-icon-dark"
                                       defaultValue={proposedCity} required
                                       list="datalistOptions" onChange={e => setProposedCity(e.target.value)}/>
                                <datalist id="datalistOptions">
                                    <option value="Одеса"/>
                                    <option value="Славське"/>
                                    <option value="Джарилгач"/>
                ...
                {results.length > 0 &&
                    <div className="results">
                        ...
                        <div className="result-cards">
                            <ExchangeApartmentItems currentItems={results} startDate={startDate} endDate={endDate}
                                            adults={adults}/>
                        </div>
                    </div>
                }
